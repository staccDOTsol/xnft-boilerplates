...
import { extendTheme, ChakraProvider, theme as chakraTheme, } from "@chakra-ui/react";
...
function primaryAlwaysLightMode(componentConfig) {
    return {
        ...
        variants: Object.entries(componentConfig.variants || {}).reduce((acc, entry) => {
            acc[entry[0]] = ({ colorMode, colorScheme, ...rest }) => runIfFn(entry[1], {
                ...rest,
                colorScheme,
        ...
        baseStyle: ({ colorMode, colorScheme, ...rest }) => runIfFn(componentConfig.baseStyle, {
            ...rest,
            colorScheme,
            colorMode: colorScheme === "primary" ? "light" : colorMode,
        }),
...
export const theme = extendTheme({
    ...
    components: {
        Button: primaryAlwaysLightMode(mergeWith({
...
export const ThemeProvider = ({ children, resetCSS = false, }) => (React.createElement(ChakraProvider, { resetCSS: resetCSS, theme: theme }, children));
