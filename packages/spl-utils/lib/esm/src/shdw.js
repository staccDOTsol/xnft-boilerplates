...
import { getMintInfo, sendAndConfirmWithRetry, sleep, toNumber, truthy, } from ".";
...
export function getStorageAccount(key, accountSeed) {
...
async function getOwnedAmount(provider, wallet, mint) {
    const ata = await Token.getAssociatedTokenAddress(ASSOCIATED_TOKEN_PROGRAM_ID, TOKEN_PROGRAM_ID, mint, wallet, true);
...
export async function initStorageIfNeeded(provider, delegateWallet, sizeBytes) {
    if (sizeBytes == 0) {
        return;
    }
    if (provider) {
        ...
        const localProvider = new AnchorProvider(connection, delegateWallet ? new NodeWallet(delegateWallet) : provider.wallet, {});
        ...
        if (shdwOwnedAmount < shdwNeeded) {
            ...
            const quote = await orcaSolPool.getQuote(shdwToken, 
            ...
            const swapPayload = await orcaSolPool.swap(pubKey, solToken, quote.getExpectedOutputAmount(), new Decimal(shdwNeeded));
            let tx = swapPayload.transaction;
            tx.recentBlockhash = (await localProvider.connection.getRecentBlockhash()).blockhash;
            tx.feePayer = pubKey;
            ...
            await sendAndConfirmWithRetry(localProvider.connection, tx.serialize(), {
                skipPreflight: true,
            }, "max");
        ...
        if (storageAccount && sizeKB && !storageAccountBigEnough) {
            await withRetries(() => shdwDrive.addStorage(accountKey, sizeKB + "KB", "v2"), 3);
        }
        else if (!storageAccount) {
            await withRetries(() => shdwDrive.createStorageAccount("chat", sizeKB + "KB", "v2"), 3);
            await withRetries(() => shdwDrive.makeStorageImmutable(accountKey, "v2"), 3);
        }
    }
}
export async function uploadFiles(provider, files, delegateWallet, tries = 5) {
    ...
    if (provider) {
        ...
        // @ts-ignore
        new Connection(getEndpoint(provider.connection), "max"), delegateWallet ? new NodeWallet(delegateWallet) : provider.wallet);
        await shdwDrive.init();
        const res = await withRetries(async () => {
            const uploaded = (await shdwDrive.uploadMultipleFiles(accountKey, 
            // @ts-ignore
...
const getDevnetWallet = () => {
    try {
        return Keypair.fromSecretKey(new Uint8Array([
